...
# Deploy 1.7.x system contract (has activate action for protocol features)
core-cli set contract eosio contracts/anvo core.system_1.7.x.wasm core.system_1.7.x.abi
...
for feature in "${protocol_features[@]}"; do
  core-cli push action eosio activate '["'"$feature"'"]' -p eosio@active
done

sleep 1

# Deploy full system contract
